...
extern "C"
{
	GuillotineBinPackC *new_GuillotineBinPack(int width, int height, int allowFlip)
	{
		return new GuillotineBinPack(width, height, allowFlip);
	}
	void delete_GuillotineBinPack(GuillotineBinPackC *s)
	{
		GuillotineBinPack *c = static_cast<GuillotineBinPack *>(s);
		delete c;
	}
	RectC GuillotineBinPack_Insert(GuillotineBinPackC *s_, int width, int height, int merge, int rectChoice, int splitMethod)
	{
		GuillotineBinPack *s = static_cast<GuillotineBinPack *>(s_);
		Rect r = s->Insert(
			width
			, height
			, merge
			, GuillotineBinPack::RectBestAreaFit//rectChoice
			, GuillotineBinPack::SplitShorterLeftoverAxis//splitMethod
		);
		RectC ret = {r.x, r.y, r.width, r.height};
